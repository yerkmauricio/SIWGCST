...
@section('content_header')
    ...
    <div class="card" style="font-family: 'Times New Roman', Times, serif;">
        <div class="card-body">
            <strong>Lista de los cargos</strong>
            @can('cargos.create')
                <a class="btn btn-success float-right" href="{{ route('cargos.create') }}">
...
@section('content')
    <h1 style="font-family: 'Times New Roman', Times, serif;">{{ ucfirst('cargo') }}</h1>
    <div class="card" style="font-family: 'Times New Roman', Times, serif;">
        <div class="card-body">
            <table class="table" id="cargo">{{-- el id producto del DATATABLE --}}
                ...
                <tbody>
                    @foreach ($cargos as $cargo)
                        <tr>
                            ...
                            <td>{{ ucfirst($cargo->nombre) }}</td>
                            <td>{{ ucfirst($cargo->descripcion) }}</td>
                            <td>{{ $cargo->salario }}0 Bs</td>
                            <td>{{ $cargo->horario }} horas laborales</td>
                            <td>{{ ucfirst($cargo->areas->nombre) }}</td>
                            <td>{{ ucfirst($cargo->n_jerarquicos->nombre) }}</td>

                            <td>
                                {{-- para eliminar es con formulario --}}
                                <form action="{{ route('cargos.destroy', $cargo) }}" method="POST" class="form-eliminar">
                                    @csrf
                                    @method('DELETE')
                                    @can('cargos.edit')
                                        <a class="btn btn-primary" href="{{ route('cargos.edit', $cargo) }}">
