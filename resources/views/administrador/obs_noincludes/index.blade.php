...
@section('content_header')
    ...
    <div class="card" style="font-family: 'Times New Roman', Times, serif;">
        <div class="card-body">
            <strong>Lista de los articulo que no incluyen en el tour</strong>
            @can('obs_noincludes.create')
                <a class="btn btn-success float-right" href="{{ route('obs_noincludes.create') }}">
...
@section('content')
    <h1 style="font-family: 'Times New Roman', Times, serif;">{{ ucfirst('articulos que no incluye el tour') }}</h1>
    <div class="card" style="font-family: 'Times New Roman', Times, serif;">
        <div class="card-body">
            <table class="table" id="obs_noinclude">{{-- el id destino lla al DATATABLE --}}
                ...
                <tbody>
                    ...
                    @foreach ($obs_noincludes as $obs_noinclude)
                        <tr>
                            <th scope="row">{{ $obs_noinclude->id }}</th>
                            <td>{{ ucfirst($obs_noinclude->nombre) }}</td>
                            <td>{{ ucfirst($obs_noinclude->descripcion) }}</td>

                            <td>

                                {{-- para eliminar es con formulario --}}
                                <form action="{{ route('obs_noincludes.destroy', $obs_noinclude) }}" method="POST"
                                    class="form-eliminar">
                                    @csrf
                                    @method('DELETE')
                                    @can('obs_noincludes.edit')
                                        <a class="btn btn-primary" href="{{ route('obs_noincludes.edit', $obs_noinclude) }}">
                                            <i class="fas fa-edit"></i>Editar
                                        </a>
                                    @endcan

                                    @can('obs_noincludes.destroy')
