...
@section('content_header')
    ...
    <div class="card" style="font-family: 'Times New Roman', Times, serif;">
        <div class="card-body">
            <strong>Lista de los niveles jerarquicos</strong>
            @can('n_jerarquicos.create')
                <a class="btn btn-success float-right" href="{{ route('n_jerarquicos.create') }}">
...
@section('content')
    <h1 style="font-family: 'Times New Roman', Times, serif;">{{ ucfirst('lista de niveles jerarquicos') }}</h1>
    <div class="card" style="font-family: 'Times New Roman', Times, serif;">
        <div class="card-body">
            <table class="table" id="n_jerarquico">{{-- el id destino lla al DATATABLE --}}
                <thead class="thead-dark">
                    <tr>
                        ...
                        <th scope="col">Numero superior</th>
                        <th scope="col">Accion</th>
                    </tr>
                </thead>
                <tbody>

                    @foreach ($n_jerarquicos as $n_jerarquico)
                        <tr>
                            <th scope="row">{{ $n_jerarquico->id }}</th>
                            <td>{{ ucfirst($n_jerarquico->nombre) }}</td>
                            <td>{{ ucfirst($n_jerarquico->descripcion) }}</td>
                            <td>{{ ucfirst($n_jerarquico->n_orden) }}</td>
                            <td>{{ ucfirst($n_jerarquico->n_superior) }}</td>
                            <td>

                                {{-- para eliminar es con formulario --}}
                                <form action="{{ route('n_jerarquicos.destroy', $n_jerarquico) }}" method="POST"
                                    class="form-eliminar">
                                    @csrf
                                    @method('DELETE')
                                    @can('n_jerarquicos.edit')
                                        <a class="btn btn-primary" href="{{ route('n_jerarquicos.edit', $n_jerarquico) }}">
                                            <i class="fas fa-edit"></i>editar
                                        </a>
                                    @endcan
                                    @can('n_jerarquicos.destroy')
